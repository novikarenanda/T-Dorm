...
<!-- END OF NAV BAR -->
  ...
  <div style="margin-left: 15%; margin-top:50px;">
    <h2 align="center" style="margin-bottom:30px;">Riwayat Tamu</h2>
    ...
    <!-- TABLE FORM RIWAYAT -->
        <table>
          ...
          <tr>
            ...
            <th style="width:120px;">Nama Tamu 1</th>
            <th style="width:120px;">Nama Tamu 2</th>
            <th style="width:120px;">Nama Tamu 3</th>
            <th style="width:30px;">Kamar</th>
          ...
          <?php 
            ...
            $sqlQuery = mysqli_query($connect, "SELECT * FROM tamu ORDER BY tanggal ASC LIMIT $mulai , $halaman"  );
                    ...
                    while($data = mysqli_fetch_array($sqlQuery)){ 
                      echo '<tr>';
                        ...
                        echo '<td>'.$data['tamu1'].'</td>';
                        echo '<td>'.$data['tamu2'].'</td>';
                        echo '<td>'.$data['tamu3'].'</td>';
                        echo '<td>'.$data['kamar'].'</td>';  //menampilkan data nama lengkap dari database
                        echo '<td>'.$data['mhs'].'</td>'; //menampilkan data kelas dari database
                        echo '<td>'.$data['tujuan'].'</td>'; //menampilkan data jurusan dari database
